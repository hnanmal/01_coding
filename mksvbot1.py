...
async def order_list(ctx):
    ...
    embd_si = discord.Embed(title="**명령어는 아래와 같아요...**", description="현재의 주가 지수를 출력합니다...", color=0x62c1cc)
    ...
    embd_si.add_field(name="*...우산*", value="`우산을 가져가야 하는지 기상청 정보 기준으로 메시지를 출력합니다...(구현중)`", inline=False)
